...
public class ArgumentsGenerator {
    ...
    public void newArguments(Dataset img1, Dataset img2, Dataset mask, File saveRoot){
        ...
        jccArguments.add(Maps.newHashMap(ImmutableMap.<String,Object>builderWithExpectedSize(8).put("dataset1", img1).put("dataset2", img2).put("significantDigits", 4).put("showIntermediates", false).put("saveFolder", new File(saveRoot.getAbsolutePath() + File.separator +"MaskNoOptions" + File.separator)).build()));
        jccArguments.add(Maps.newHashMap(ImmutableMap.<String,Object>builderWithExpectedSize(8).put("dataset1", img1).put("dataset2", img2).put("significantDigits", 4).put("showIntermediates", true).put("saveFolder", new File(saveRoot.getAbsolutePath() + File.separator +"MaskWithOptions" + File.separator)).build()));

        gaussArguments = new ArrayList<>();
        jccArguments.forEach((argument) ->
                gaussArguments.add((HashMap<String, Object>) argument.clone()));
        gaussArguments.forEach((argument) ->
                argument.put("maskDataset", mask));
        ...
        gaussArguments.get(0).put("maskAbsent", true);
        gaussArguments.get(0).put("maskAbsent", true);
        gaussArguments.get(0).put("maskAbsent", false);
        gaussArguments.get(0).put("maskAbsent", true);
        gaussArguments.get(0).put("maskAbsent", true);
        gaussArguments.get(0).put("maskAbsent", false);
        gaussArguments.get(0).put("maskAbsent", false);
